...
uv run python scripts/prepare_bus_overlay.py \
  ...
  --rail-naptan-xml build/bus-overlay/naptan-national-rail.xml \
...
The optional `--rail-naptan-xml` input adds active `GRLS` train-station groups from the [national rail NaPTAN feed (area 910)](https://naptan.api.dft.gov.uk/v1/access-nodes?atcoAreaCodes=910&dataFormat=xml), clipped to the same boundary. Download that XML separately from the local-authority bus extract; station entrances are not substituted for station groups. The [DfT schema guide](https://naptan.dft.gov.uk/naptan/schema/2.5/doc/NaPTANSchemaGuide-2.5-v0.67.pdf) defines `GRLS` as a rail station. Records retain their source identity, coordinates and dates; they do not establish current train services. The viewer includes these in **Bus stops & train stations**, with blue train symbols and the existing orange stars for bus facilities and transfer points. The visible key and info tooltip explain both symbols, and clicking a train opens its station details.
