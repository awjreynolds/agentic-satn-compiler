...
### WECA build and urban-anchor diagnosis

The first local WECA journey build (`run-dd89feec6238`, `build/weca-journey-recovery`) completed in 2,117.64 seconds (35 minutes 18 seconds). It produced 21 selected interurban journeys and 58 alternatives. It was not acceptance: Bath, Kingswood and Yate were recognised as canonical places but had no urban adjacency, while strategic publication incorrectly reported complete with zero gaps.

The loss occurred during place binding, before route selection. The reciprocal routing graph's largest component contained 99,340 of 113,096 nodes (87.84%), below the existing generic nearest-node filter's 90% switch. Bath therefore attached to a 10-node pedestrian fragment 36.79 m away; Kingswood and Yate attached to pedestrian fragments of two and eight nodes. In B&NES the same generic filter activated because its largest component contained 91.47% of nodes, attaching Bath to the connected node 40.74 m away. Increasing the regional extent thus changed which component received the same town anchor.

The compiler's raw WECA browser payload was approximately 642 MB and crashed headless Chromium. The existing progressive area packager reduced startup data to 53 MiB and rendered the local map without page errors. Main retains all semantic journeys while displaying each exact shared segment once. Round caps and joins prevent the individual display segments from looking like dashed linework.

The repair gives canonical urban anchoring an explicit preference for the largest reciprocal component within the existing configured urban attachment extent (`urban_scope_buffer_km`), independent of the generic 90% switch. Outside that preference, it retains the nearest local reciprocal component. Generic routing callers are unchanged. Required place-coverage issues now survive corridor preparation, planning and publication as one-place, one-point gaps with source identity; an unresolved place changes strategic status to `complete-with-gaps`. Actual route candidates retain their two-place endpoint contract. Cache revision `poc-v9` prevents ordinary reuse of the earlier semantic result.

Focused public preparation/compiler/gap tests passed (10), including the nearby fragment and a remote local fragment when the dominant component exceeds 90%; the generic nearest-node regression also passed. Two pre-existing compilation-dependency tests still expect `poc-v2` and were not expanded into this build's scope. The further proposal to redefine binding where no reciprocal node exists anywhere within the urban extent was excluded from this bounded WECA repair; acceptance checks the actual regional town bindings instead.

The corrected strategic stages were replayed from the exact retained WECA routable edges and source snapshot, preserving the existing access, school and topography work. The derived local bundle is `build/weca-journey-recovery/strategic-replay/compiled-network-bundle`; `strategic-rebuild.json` explicitly records that this is a partial strategic rebuild of `run-dd89feec6238`, not another full compilation. Preparation, selection, dependent asset accounting and normal publication completed in **1,196.56 seconds (19 minutes 57 seconds)**. The original retained artifact remains intact.

The rebuilt result has **28 selected interurban journeys, 78 alternatives, all 16 canonical towns/cities in one selected endpoint component, zero urban preparation issues and zero strategic journey gaps**. All actual town bindings are within 78.90 m. Bath now binds to node `1351317672` at 40.74 m, Kingswood to `265452321` at 59.24 m and Yate to `7872684091` at 70.64 m. Bath–Radstock retains the same 19.91 km cycling-led choice and 14.65 km A-road alternative as the B&NES proof; Bath–Keynsham retains the 11.60 km cycling-led choice. Bath also has a selected 17.95 km cycling-led journey to Kingswood. Bath and Radstock both connect through the selected network to Bristol. Endpoint-chain checks prove connectivity, not a separately optimised end-to-end itinerary.

The normal strategic publication has zero findings. Its 81 Main display features contain exactly the selected routes' 13,614 unique physical segments, with no duplicated, missing or added segments. These checks do not claim that the retained access and school obligations are all resolved. The local evidence files are `urban-journeys-rebuilt.json`, `journey-checks-rebuilt.json` and `rebuild-verification.json` under `build/weca-journey-recovery`.

Independent Sol review passed the actual WECA result: all selected sets retain alternatives and selection reasons. Bath–Kingswood's selected route uses five source IDs explicitly named **Bristol & Bath Railway Path** (`1361726993`, `31171896`, `38757886`, `5973056`, `999677349`) and retains current-NCN evidence. This confirms named asset participation without claiming an exact NCN route number from generic NCN evidence.

The real progressive browser map rendered all 28 selected journeys without page errors. Clicking a shared section offered Bath–Kingswood, Bath–Keynsham, Bath–Chipping Sodbury and Radstock–Bath. Choosing Radstock–Bath opened its three related alternatives with the global Alternatives switch still off; closing cleared the comparison. `shared-browser-verification.json` and the accompanying screenshots record this check. The release remains a local build; this increment was not published to GitHub Pages.
