...
Complete the requested outcome and validate the acceptance criteria that establish it. Keep changes and checks tied to that scope. Resolve concrete failures and newly discovered blockers before declaring completion; report any blocker that requires an external decision or unavailable dependency. Once the outcome is demonstrated, report the result and relevant validation.

Use limits and thresholds from the request, applicable technical contracts, project policy or measured evidence. State their authority or derivation; examples and illustrative proportions are not policy defaults. Ask for unresolved owner decisions that affect the outcome.
...
The GitNexus index is named **banes-satn**. Use it when graph-based navigation helps answer the current code question.
